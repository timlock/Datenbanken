...
<!--
    Das Stylesheet behandelt eine Generalisierung nach Option C. Dieses Stylesheet wird
    auch als Default für disjunkte Generalisierungen verwendet. Die Generalisierung
    ...
    ein Typ-Attribut ergänzt wird. Beziehungstypen oder andere Generalisierungen, an denen
    eine der Subentitätstypen beteiligt ist, müssen so angepasst werden, dass stattdessen
    der Superentitätstyp daran teilnimmt.
    
    Das Stylesheet ist darauf ausgerichtet, in das Stylesheet EER2ER.xsl eingebunden zu
    werden. Alle Templates in diesem Stylesheet werden im Modus "t3" ausgeführt, um
...
<xsl:stylesheet xmlns="&eer;" xmlns:eer="&eer;" xmlns:func="&func;"
    ...
    -->
    <xsl:template match="eer" mode="t3">
    ...
    <!--
        ...
        angehören.
    -->
    <xsl:template match="attribute[func:getOwner(@id) = $subEntities/@id]" mode="t3">
        <xsl:copy>
    ...
    <!--
        ...
        der Subentitätstypen ergänzt werden. Außerdem wird ein Typ-Attribut erzeugt und dem
        Superentitätstyp hinzugefügt. Beachte: Auch das node- und attributeEdge-Element für
        das Typ-Attribut müssen erstellt werden.
    -->
    <xsl:template match="entity[@id = $superEntity/@id]" mode="t3">
        <xsl:variable name="typeAttribute">
            ...
            <attribute id="{$newId}" key="false" derived="false" multivalued="false">
                ...
                <domain>
                    ...
                    <restrictions>
                        <xsl:if test="xs:boolean($generalization/@total)">
                            <notNull/>
                        </xsl:if>
                        <!--
                        <minimum value="1"/>
                        <maximum value="{count($subEntities)}"/>
                        -->
                    </restrictions>
                </domain>
            </attribute>
            ...
            <node idref="{$newId}">
                <pointAnchor x="{func:getX($superEntity/@id)}" y="{func:getY($superEntity/@id) +
                    1.5}"/>
            </node>
            <attributeEdge>
                <from>
        ...
        <xsl:copy>
            ...
            <attributes>
                ...
                <xsl:copy-of select="$typeAttribute/attributeRef"/>
            </attributes>
        </xsl:copy>
        <xsl:copy-of select="$typeAttribute/attribute | $typeAttribute/node |
            $typeAttribute/attributeEdge"/>
    </xsl:template>
    <!--
        Entfernt die entity-Elemente der Subentitätstypen.
    -->
    <xsl:template match="entity[@id = $subEntities/@id]" mode="t3"/>
    <!--
    ...
    -->
    <xsl:template match="generalization[@id = $generalization/@id]" mode="t3"/>
    <!--
    ...
    -->
    <xsl:template match="businessRule[@about = $subEntities/@id]" mode="t3"/>
    <!--
        Entfernt die node-Elemente der umzuwandelnden Generalisierung und der
        Subentitätstypen.
    -->
    <xsl:template match="node[@idref = $generalization/@id or @idref = $subEntities/@id]" mode="t3"/>
    <!--
        Transformiert die node-Elemente der Attribute der Subentitätstypen. Diese werden
        in Richtung Superentitätstyp verschoben.
    -->
    <xsl:template match="node[func:getOwner(@idref) = $subEntities/@id]" mode="t3">
    ...
    -->
    <xsl:template match="relationshipEdge[.//@idref = $subEntities/@id]" mode="t3">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="#current"/>
            <xsl:attribute name="total" select="xs:boolean(@total) and
                xs:boolean($generalization/@total) and count($subEntities) = 1"/>
    ...
    -->
    <xsl:template match="generalizationEdge[.//@idref = $generalization/@id]" mode="t3"/>
    <!--
        Entfernt alle specializationEdge-Elemente der umzuwandelnden Generalisierung.
    -->
    <xsl:template match="specializationEdge[.//@idref = $generalization/@id]" mode="t3"/>
    <!--
    ...
    -->
    <xsl:template match="nodeAnchor[.//@idref = $subEntities/@id]" mode="t3">
        <xsl:copy>
            <xsl:attribute name="idref" select="$superEntity/@id"/>
    ...
    -->
    <xsl:template match="@* | node()" mode="t3">
        <xsl:copy>
            <xsl:apply-templates select="@* | node()" mode="#current"/>
        </xsl:copy>
    </xsl:template>
